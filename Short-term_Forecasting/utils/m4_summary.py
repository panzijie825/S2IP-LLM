...
def group_values(values, groups, group_name):
    ...
    selected_values = [values[i] for i in range(len(values)) if mask[i]]
    filtered_values = [np.array(v)[~np.isnan(v)] for v in selected_values]
    

    return np.array(filtered_values, dtype=object)
#np.array([v[~np.isnan(v)] for v in values[groups == group_name]])
#
